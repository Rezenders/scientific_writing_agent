...
<!-- CUSTOMIZE: Add any project-specific terminology rules here.
     Example:
     - "Action" has three distinct meanings in this manuscript: [explain]. Keep them distinct.
     - Refer to the system's reasoning component as "reasoner", not "engine" or "inference engine".
-->
...
<!-- CUSTOMIZE: If using Docker for reproducible builds, document it here:
     docker build -t paper .
     docker run --rm -u $(id -u):$(id -g) -v $(pwd):/paper paper
-->

<!-- CUSTOMIZE: List any suppressed chktex warnings and why:
     chktex suppresses warnings X (reason), Y (reason) — consistent false positives.
-->

<!-- CUSTOMIZE: List local build dependencies:
     Required: latexmk, chktex, [other tools]
     Install: sudo apt install latexmk chktex ...
-->
...
<!-- ADD YOUR AUTHORS: Add per-author annotation commands here.
     Example:
     | `\alice{text}` | cyan | Alice's inline comments |
     | `\bob{text}` | orange | Bob's inline comments |
-->
...
<!-- CUSTOMIZE: Document your project-specific LaTeX macros here.
     Example:
     - `\approachname` → `MySystem`
     - `\metricname` → `F1`
     These macros must be preserved verbatim in all agent rewrites.
-->

## Implementation Reference (OPTIONAL)

<!-- OPTIONAL: Only include this section if your paper describes a software implementation.
     Fill in the paths so that agents know where to verify technical claims. -->
...
<!-- OPTIONAL: Document any implementation behaviors that are commonly misdescribed in prose.
     These serve as a checklist for the writer and implementation checker agents.
     Example:
     - `applicable_tactics` selects rules where the target is X OR Y — the X-only branch is easy to omit.
     - `interface_hosts` covers three interface types: A, B, and C — descriptions listing only two are incomplete.
-->
